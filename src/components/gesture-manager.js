...
class GestureManager {
    constructor(options, handlers, disabledSwipeKeys, multiPressableKeys) {
        ...
        this.popoverStateMachine = new PopoverStateMachine({
            ...
            onClick: (keyId, domNodeId, inPopover) => {
                if(domNodeId === KeyTypes.MANY) {
                    let popTime = setTimeout(() => {
                        let obj = {
                            focus: null,
                            popover: {
                                childIds: this.manyChildIds,
                                parentId: KeyTypes.MANY
                            }
                        }
                        const {popover, ...rest} = obj;
                        handlers.onActiveNodesChanged({
                            popover: popover && {
                                parentId: popover.parentId,
                                bounds: this.nodeManager.layoutPropsForId(
     ...
     */
    onClickStart(evt, id) {
        // console.log("gesture-manager: onClickStart", this.trackEvents, this.isPc);
        if (!this.trackEvents || !isPc) {
            return;
        }
     ...
     */
    onTouchCancel(evt) {
        // console.log("gesture-manager: onTouchCancel")
        if (!this.trackEvents) {
            return;
        }
