...
// LatexCmds['\u2260'] = LatexCmds.ne = LatexCmds.neq = bind(BinaryOperator,'\\ne ','&ne;');
  ...
  LatexCmds['≈'] = LatexCmds['\u2248'] = LatexCmds.asymp = LatexCmds.approx = bind(BinaryOperator,'\\approx ','&asymp;');
...
//various symbols
  ...
  LatexCmds.not = //bind(Symbol,'\\not ','<span class="not">/</span>');
    LatexCmds['¬'] = LatexCmds['\u00ac'] = LatexCmds.neg = bind(VanillaSymbol,'\\neg ','&not;');

  LatexCmds['…'] = LatexCmds['\u2026'] = LatexCmds.dots = LatexCmds.ellip = LatexCmds.hellip =
...
//Greek constants, look best in non-italicized Times New Roman
  ...
  //add Parallelogram
  LatexCmds.lozenge = P(Symbol, function(_, super_) {
    _.init = function() {
      super_.init.call(this, '\\lozenge ', '<span class="mq-lozenge-parallelogram">&lozenge;</span>');
    };
  });
  // bind(NonSymbolaSymbol,'\\lozenge ','&lozenge;');
...
// [4]: http://en.wikipedia.org/wiki/Windows-1252
  LatexCmds['¹'] = LatexCmds['\u00b9'] = bind(LatexFragment, '^1');
  LatexCmds['²'] = LatexCmds['\u00b2'] = bind(LatexFragment, '^2');
  LatexCmds['³'] = LatexCmds['\u00b3'] = bind(LatexFragment, '^3');
  LatexCmds['¼'] = LatexCmds['\u00bc'] = bind(LatexFragment, '\\frac14');
  LatexCmds['½'] = LatexCmds['\u00bd'] = bind(LatexFragment, '\\frac12');
  LatexCmds['¾'] = LatexCmds['\u00be'] = bind(LatexFragment, '\\frac34');
  ...
  var PlusMinus = P(BinaryOperator, function(_) {
    ...
    _.contactWeld = _.siblingCreated = _.siblingDeleted = function(opts, dir) {
      ...
      // this.jQ[0].className = (!this[L] || this[L] instanceof BinaryOperator ? '' : 'mq-binary-operator');
      //add
...
//yes, these are different dashes, I think one is an en dash and the other is a hyphen
  ...
  LatexCmds['±'] = LatexCmds['\u00b1'] = LatexCmds.pm = LatexCmds.plusmn = LatexCmds.plusminus =
  ...
  LatexCmds.dot = P(MathCommand, function(_, super_) {
    _.init = function() {
      super_.init.call(this, '\\dot', '<span class="mq-non-leaf" ><span class="mq-dot-recurring-inner">'
        + '<span class="mq-diacritic-above" >&#x2d9;</span>'
...
//add
  ...
  LatexCmds.overline = LatexCmds.bar = P(MathCommand, function(_, super_) {
    _.ctrlSeq = '\\overline';
    _.htmlTemplate =
      '<span class="mq-non-leaf">'
      +   '<span class="mq-diacritic-above mq-overline"></span>'
      +   '<span class="mq-diacritic-stem">&0</span>'
      + '</span>'
    ;
    _.textTemplate = ['overline(', ')'];
  });
  LatexCmds.coordinate = P(MathCommand, function(_, super_) {
    _.htmlTemplate =
      '<span class="mq-coordinate mq-non-leaf">'
      +   '<span class="mq-paren">(</span>'
      +   '<span class="mq-non-leaf">'
      +     '<span class="mq-array-inline mq-non-leaf">'
      +       '<span>&0</span>'
      +       '<span>,</span>'
      +       '<span>&1</span>'
      +     '</span>'
      +   '</span>'
      +   '<sup class="mq-paren">)</sup>'
      + '</span>'
    ;
    _.textTemplate = ['((' , '))'];
  });
...
//   _.htmlTemplate =
//       '<span class="mq-non-leaf">'
//     // +   '<span class="mq-diacritic-above mq-overarrow"></span>'
//     +   '<span class="mq-diacritic-stem mq-overarrow">&0</span>'
//     + '</span>'
//   ;
//   _.textTemplate = ['vec(', ')'];
// });
    ...
    _.createLeftOf = function(cursor) {
      if (!this.replacedFragment) { // unless wrapping seln in brackets,
        ...
        if (this.sides[L].ch === '|') { // check both sides if I'm a pipe
          var brack = this.oppBrack(opts, R, cursor[R])
            || this.oppBrack(opts, L, cursor[L])
            || this.oppBrack(opts, 0, cursor.parent.parent);
        }
        else {
          var brack = this.oppBrack(opts, -this.side, cursor[-this.side])
            || this.oppBrack(opts, -this.side, cursor.parent.parent);
        }
        //
      }
      if (brack) {
